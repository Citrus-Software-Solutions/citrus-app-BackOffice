...
public class JobOfferMapperNest implements JobOfferMapper {

	@Override
	public List<JobOffer> getJobOffers(APIPort port) {
		List<JobOffer> offers = new ArrayList<>();
		
		for (JsonNode n: port.requestGetList("job-offers")) {
	...
	public JobOffer getJobOffer(APIPort port, long id) {
		...
		var node = port.requestGet("job-offers/" + String.valueOf(id));
		...
		offer.setAddress(new Address(
				...
				new Zip(node.get("location").get("zip").asText())));
